...
{
	//to insert billing record
	$sql = "INSERT INTO billing(sellerid,date,status) VALUES('$_POST[seller]','$_POST[purdate]','Active')";
		if(!mysqli_query($connection,$sql))
		{
			echo "Error in insert statement". mysqli_error($connection);
		}
		$billid = mysqli_insert_id($connection);
		//to update purchase records
		$sql = "UPDATE purchase SET status='Active',billingid='$billid' WHERE billingid='0' AND status='Inactive'";
 		if(!mysqli_query($connection,$sql))
		{
		...
		else
		{
			...
			echo "<script>window.location.assign('purchasereport.php?billid=".$billid."')</script>";
		}
}
//to retrieve records
...
?>
            ...
            <h2>Mpesa Payment</h2>
            <form action="purchasereportview.php" method="POST">
